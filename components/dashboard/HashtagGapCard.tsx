...
const recColors: Record<string, string> = {
  adopt: "bg-neon-green/10 text-neon-green border-neon-green/20",
  ignore: "bg-white/5 text-muted-foreground border-white/10",
  already_using: "bg-brand-600/10 text-brand-300 border-brand-600/20",
};
  ...
  return (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold flex items-center gap-2">
        <Hash className="w-4 h-4 text-brand-400" /> Hashtag gap analysis
      </h4>
      {toAdopt.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs text-neon-green font-medium">Recommend adopting:</p>
          {toAdopt.map((g) => (
            <div key={g.hashtag} className="flex items-start gap-2 text-xs">
              <ArrowRight className="w-3 h-3 text-neon-green flex-shrink-0 mt-0.5" />
              <div>
                <span className="font-mono text-neon-green">{g.hashtag}</span>
